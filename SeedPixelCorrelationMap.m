...
function SeedPixelCorrelationMap(DataFolder, Seedname, ManualInput, StartFrame, EndFrame)
...
% if exist([pathFixed filesep 'ROI_149.mat'], 'file')
%     load([pathFixed filesep 'ROI_149.mat']);
% else
%     disp('??? I dont get it either, no ROI_149 somehow?')
%     return
% end
% clear idx
...
if( exist([DataFolder 'FIXTHIS.png'], 'file') && ManualInput == 0 )
    disp('Seed pixel correlation map already done, function exited')
    return
elseif( exist([DataFolder 'FIXTHIS.png'], 'file') && ManualInput == 1 )
    disp('Seed pixel correlation map already done, OVERWRITING FILES')
end 

% if ~exist('Seedname', 'var')
...
% Get centroid of ROI based on weight
    ...
    saveas(gcf, ['/media/mbakker/data1/Hypoxia/SeedPixelCorrMap/' MouseAcq '.tiff']);    
    close gcf
