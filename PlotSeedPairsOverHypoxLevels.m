...
function PlotSeedPairsOverHypoxLevels(SeedPairs, GSR)
...
for index = 1:size(SeedPairs, 2)
    ...
    for ind = 1:size(HypoxiaLevels, 2)
        %         load(['/media/mbakker/data1/Hypoxia/CorrMatrix/' HypoxiaLevels{ind} '_Hypox.mat']);
        if GSR == 1
            load(['/media/mbakker/data1/Hypoxia/CorrMatrix/ForStats/' HypoxiaLevels{ind} '_Hypox.mat']);
        else
            load(['/media/mbakker/data1/Hypoxia/CorrMatrix/ForStats/' HypoxiaLevels{ind} '_Hypox_NoGSR.mat']);
        end
        SeedPairCorr = mean(CorrHypox(Seedindex1, Seedindex2, :),3, 'omitnan');
        Correlations(ind) = SeedPairCorr;
        Stdevs(ind) = std(CorrHypox(Seedindex1, Seedindex2,:), 'omitnan');
        SEMs(ind) = Stdevs(ind)/sqrt(length(CorrHypox(Seedindex1, Seedindex2,:)));
    end
...
if GSR == 1
    saveas(gcf, ['/media/mbakker/data1/Hypoxia/SeedPairs/SeedPairCorr_Pos_HypoxLevels.eps'], 'epsc');
    saveas(gcf, ['/media/mbakker/data1/Hypoxia/SeedPairs/SeedPairCorr_Pos_HypoxLevels.tiff'], 'tiff');
else
    saveas(gcf, ['/media/mbakker/data1/Hypoxia/SeedPairs/SeedPairCorr_Pos_HypoxLevels_NoGSR.eps'], 'epsc');
    saveas(gcf, ['/media/mbakker/data1/Hypoxia/SeedPairs/SeedPairCorr_Pos_HypoxLevels_NoGSR.tiff'], 'tiff');
end
close all;
...
if GSR == 1 % without GSR there's no negative
    ...
    saveas(gcf, ['/media/mbakker/data1/Hypoxia/SeedPairs/SeedPairCorr_Neg_HypoxLevels.eps'], 'epsc');
