...
if( ManualInput == 0 ) && ( exist('/media/mbakker/data1/Hypoxia/spO2/Hypox_8_1-linegraph-mean.fig', 'file') )
    disp('spO2 plotting already done, exited function')
    return;
end
if( ManualInput == 1 ) && ( exist('/media/mbakker/data1/Hypoxia/spO2/Hypox_8_1-linegraph-mean.fig', 'file') )
...
for index = 1:size(HypoxiaLevels, 2)
    ...
    axes1 = axes;
    hold(axes1,'on');
    set(axes1,'FontSize',20,'FontWeight','bold','LineWidth',2); %fontsize was 14
...
%     title([TitleList{index} ' - Median'])
    ...
    saveas(gcf, ['/media/mbakker/data1/Hypoxia/spO2/' HypoxiaLevel '-linegraph-median.tiff'])
    saveas(gcf, ['/media/mbakker/data1/Hypoxia/spO2/' HypoxiaLevel '-linegraph-median.fig'])
    ...
    ySEM = std(spO2Hlevel, 1, 'omitnan')/sqrt(8);
    ySEM = movmedian(ySEM, 200);
    CplotsI95 = tinv([0.025 0.975], 8-1);
    yCI95 = bsxfun(@times,ySEM, CI95(:));
...
%     title([TitleList{index} ' - Mean'])
    ...
    %Save
    saveas(gcf, ['/media/mbakker/data1/Hypoxia/spO2/' HypoxiaLevel '-linegraph-mean.tiff'])
